...
message("setup knitr options")

#knitr::opts_chunk$set(echo=TRUE, warning=TRUE, message=TRUE, cache=TRUE, dev=c('png','pdf'), fig.width=8, fig.height=8)
#knitr::opts_chunk$set(echo=TRUE, warning=FALSE, message=FALSE, cache=FALSE, dev=c('png','pdf'), fig.width=8, fig.height=8)
#knitr::opts_chunk$set(echo=TRUE, warning=FALSE, message=FALSE, cache=FALSE, dev=c('pdf'), fig.width=8, fig.height=8)
knitr::opts_chunk$set(echo=TRUE, warning=FALSE, message=FALSE, cache=FALSE, dev=c('svg'), fig.width=8, fig.height=8)
...
annot_config <- readr::read_tsv(file="bin/annotation.tsv")
units_config <- readr::read_tsv(file="bin/units.tsv")
#
# running this from snakemake, so
# try using the snakemake object
#
orgdb <- annot_config %>% filter(common_name == snakemake@config[["common_name"]]) %>% select(org.db)
orgdbname <- unlist(orgdb)
library(orgdbname, character.only=TRUE) # load the org.db for your organism
library(AnnotationDbi)

#units_cols <- colnames(units_config)
#factor1 <- units_cols[2]
#factor2 <- units_cols[3]
factor1 <- snakemake@config[["factorA_name"]]
factor2 <- snakemake@config[["factorB_name"]]

factors <- c(factor1, factor2)

#message(paste0("factors: '", factor1, "', '", factor2, "'"))
message(paste0("factors: '", factors, "'\n"))
...
ext_data_dir <- "/analysis/star"
wd <- getwd()
ext_data_dir <- paste0(wd, "/", ext_data_dir)
message(paste("ext_data_dir: ", ext_data_dir))
count_data <- bbcRNA::star_to_mat(dir = ext_data_dir,
                                  rgx = "^[^\\.]+", column = 3)
dim(count_data)

message(paste("dim of count_data: ", c(dim(count_data),"\n")))
message(paste("row names:"))
message(head(rownames(count_data)))
message(paste("col names:"))
message(colnames(count_data))

# add gene symbols
gene_names_df <- data.frame(row.names = rownames(count_data))
gene_names_df$Symbol <- AnnotationDbi::mapIds(eval(as.name(orgdbname)), rownames(gene_names_df), 
                                              keytype="ENSEMBL", column="SYMBOL", 
                                              multiVals="first")
gene_names_df$Uniq_syms <- scater::uniquifyFeatureNames(rownames(gene_names_df), gene_names_df$Symbol)
gene_names_df$entrez <- AnnotationDbi::mapIds(eval(as.name(orgdbname)), rownames(gene_names_df), 
...
samp_names
#data_for_DE <- tibble(
data_for_DE <- data.frame(
#                        Sample = samp_names,
#                        Genotype = str_extract(samp_names, "^[^_]+"),
#                        Litter = str_extract(samp_names, "\\d+$")
                        FactorA = str_extract(samp_names, "^[^_]+"),
                        FactorB = sapply(str_split(samp_names, "_"), "[[", 2)
                        )
#                     ) %>%
#  tibble::column_to_rownames("Sample")

#message(paste("data_for_DE 00", data_for_DE))
#message(paste("rownames for data_for_DE 00", rownames(data_for_DE)))

data_for_DE <- remove_rownames(data_for_DE)
row.names(data_for_DE) <- samp_names

#message(paste("data_for_DE 01", data_for_DE))
#message(paste("rownames for data_for_DE 01", rownames(data_for_DE)))

colnames(data_for_DE) <- factors

data_for_DE %>% DT::datatable()

message(paste("data_for_DE 02", data_for_DE))
message(paste("rownames for data_for_DE 02", rownames(data_for_DE)))
...
#design <- model.matrix(~0+FactorA+FactorB, data = y$samples)
#design <- model.matrix(~0+factors[1]+factors[2], data = y$samples)
fmla <- as.formula(paste("~0","+",factor1,"+",factor2))
#design <- model.matrix(~0+Genotype+Treatment, data = y$samples)
design <- model.matrix(fmla, data = y$samples)
design
...
#plot_pca(norm_counts, "pca.rds", color="FactorB", pch="FactorA")
plot_pca(norm_counts, "pca.rds", color=factors[2], pch=factors[1])
```
...
#message(paste("y$samples$FactorA", y$samples$FactorA))
message("making contrasts")
message(paste("y$samples[[1]]", y$samples[[1]]))
...
contrasts_as_str <- sapply(as.data.frame(combn(unique(paste0("FactorA", y$samples$FactorA)), 2)), function(comp) paste0(comp[1],"-",comp[2]))
#contrasts_as_str <- sapply(as.data.frame(combn(unique(paste0(factor1, y$samples$FactorA)), 2)), function(comp) paste0(comp[1],"-",comp[2]))

message(paste("contrasts_as_str: ", contrasts_as_str))
